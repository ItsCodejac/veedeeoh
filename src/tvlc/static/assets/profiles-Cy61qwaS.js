const E="veedeeoh_active_profile",m=[{id:"default_main",name:"Main Profile",avatar_color:"#c5f04e",is_kids:!1,max_rating:"TV-MA"},{id:"default_kids",name:"Kidz",avatar_color:"#ff5e7e",is_kids:!0,max_rating:"PG"}];function v(){try{const e=localStorage.getItem("veedeeoh_household_profiles");if(!e)return m;const r=JSON.parse(e);return r.length>0?r:m}catch{return m}}function P(e){localStorage.setItem("veedeeoh_household_profiles",JSON.stringify(e))}function _(){try{const r=localStorage.getItem(E);if(r){const t=JSON.parse(r);if(t&&t.id)return t}}catch{}return v()[0]||m[0]}function z(e){localStorage.setItem(E,JSON.stringify(e)),document.documentElement.setAttribute("data-kids-mode",e.is_kids?"true":"false"),window.dispatchEvent(new CustomEvent("veedeeoh:profile-changed",{detail:e}))}function I(e,r){const t=document.getElementById("pinVerificationModal");t&&t.remove();const n=document.createElement("div");n.id="pinVerificationModal",n.style.cssText=`
  ...
  `,document.body.appendChild(n);const o=n.querySelector("#pinInput"),c=n.querySelector("#pinError"),p=n.querySelector("#cancelPinBtn"),d=n.querySelector("#submitPinBtn"),i=()=>{o.value===e?(n.remove(),r()):(c.style.display="block",o.value="",o.focus())};d.onclick=i,p.onclick=()=>n.remove(),o.onkeyup=l=>{l.key==="Enter"&&i(),o.value.length===4&&i()}}function B(e){const r=document.getElementById("profileSwitcherModal");r&&r.remove();const t=v(),n=_(),o=document.createElement("div");o.id="profileSwitcherModal",o.style.cssText=`
      ...
      <div style="display: flex; gap: 24px; justify-content: center; flex-wrap: wrap; margin-bottom: 40px;">
        ${t.map(i=>`
          ...
          ">
            ...
            ">
              ${i.is_kids?"🎈":i.name.charAt(0).toUpperCase()}
              ${i.pin?'<span style="position: absolute; bottom: 6px; right: 6px; font-size: 14px;">🔒</span>':""}
            </div>
  ...
  `,document.body.appendChild(t);let l=c;const s=t.querySelectorAll(".colorChoiceBtn");s.forEach(a=>{a.addEventListener("click",()=>{s.forEach(u=>u.style.border="none"),a.style.border="3px solid #fff",l=a.dataset.color||"#c5f04e"})});const f=t.querySelector("#saveProfileBtn"),x=t.querySelector("#cancelEditBtn");x&&x.addEventListener("click",()=>t.remove()),f&&f.addEventListener("click",()=>{const a=t.querySelector("#editName").value.trim(),u=t.querySelector("#editKids").checked,h=t.querySelector("#editRating").value,k=t.querySelector("#editPin").value.trim();if(!a){alert("Please enter a profile name.");return}const b=v();if(n&&e){const g=b.findIndex(S=>S.id===e.id);g!==-1&&(b[g]={id:e.id,name:a,avatar_color:l,is_kids:u,max_rating:h,pin:k||null})}else{const g={id:"profile_"+Date.now(),name:a,avatar_color:l,is_kids:u,max_rating:h,pin:k||null};b.push(g)}P(b),t.remove(),B()})}function y(e){return e.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}export{_ as getActiveProfile,v as getStoredProfiles,w as openProfileEditor,B as openProfileSwitcher,I as promptPinVerification,P as saveProfiles,z as setActiveProfile};
