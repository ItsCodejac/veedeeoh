...
export interface PartyOptions {
  ...
  isPublic?: boolean;
  blurb?: string | null;
  // A SNAPSHOT OF THE TITLE, taken here because this is the only place it
  // exists on our side. The catalogue is resolved per viewer and never reaches
  // the server, so genre, rating, decade and language cannot be joined to at
  // query time -- and without them the directory's four facet filters have
  // nothing to filter on. Runtime is here for the same reason: "ending soonest"
  // is not answerable without knowing how long the thing is.
  genre?: string | null;
  rating?: string | null;
  decade?: string | null;
...
export async function createParty(opts: PartyOptions): Promise<{ joinCode: string; link: string; partyId: string }> {
  ...
  const { data: created, error } = await getSupabase().from("parties").insert({
    ...
    runtime_mins: opts.runtimeMins ?? null,
  }).select("id").single();
  // One party at a time, enforced by a partial unique index rather than by
  // everyone remembering. A host who already has a room open gets told to go
  // back to it, which is the only thing this call could honestly offer them.
  if (error?.code === "23505") throw new Error("ALREADY_HOSTING");
...
export function kickViewer(userId: string, reason = "fit"): void {
  ...
  // The relay's ban list lives in the Durable Object, and the object closes
  // five minutes after the room goes idle -- so the strongest action a host has
  // used to expire with the party. The standing decision is written here, by
  // the host's own client, because RLS already lets exactly one account write
  // it and that keeps the relay free of any credential that could.
  if (reason === "fit" || reason === "conduct") {
    void (async () => {
      const { data: u } = await getSupabase().auth.getUser();
      if (!u.user) return;
      const { error } = await getSupabase().from("party_blocks").upsert({
        host_user_id: u.user.id,
        blocked_user_id: userId,
        reason,
        party_title: partyTitle || null,
      }, { onConflict: "host_user_id,blocked_user_id" });
      if (error) console.warn("[party] block", error);
    })();
  }
}

/** Host lifts a block: the durable row, its appeal, and the relay's local copy
 *  for the room that is open right now. */
export async function unblockUser(userId: string): Promise<boolean> {
  const { data, error } = await getSupabase().rpc("unblock_user", { subject: userId });
  if (error) { console.warn("[party] unblock", error); return false; }
  // Best effort, and only meaningful while a room is actually open.
  try { socket?.send(JSON.stringify({ type: "unban", userId })); } catch {}
...
export interface PublicParty {
  ...
  genre: string | null; rating: string | null;
...
/** What the filter bar can ask for. Everything here is answered in the
 *  database, not here: filtering client-side means shipping the whole
 *  directory to every viewer, which is fine at twelve rows and wrong at twelve
 *  hundred. */
export interface PartyFilters {
  ...
  decade?: string;
  language?: string;
  /** 's' under five watching, 'm' five to fifteen, 'l' over fifteen. */
  size?: string;
  freeSeats?: boolean;
  justStarted?: boolean;
  followsOnly?: boolean;
  hasSocial?: boolean;
  sort?: "new" | "most" | "ending";
}

/** Parties whose hosts chose to be listed.
 *
 *  Sort is always follows-first and then the chosen order -- two clauses in one
 *  query, not a separate one for followed hosts. */
export async function listPublicParties(f: PartyFilters = {}): Promise<PublicParty[]> {
  const { data, error } = await getSupabase().rpc("public_parties", {
    q: f.q || null,
    f_genre: f.genre || null,
    f_rating: f.rating || null,
    f_decade: f.decade || null,
    f_language: f.language || null,
    f_size: f.size || null,
    free_seats: !!f.freeSeats,
    just_started: !!f.justStarted,
    follows_only: !!f.followsOnly,
    has_social: !!f.hasSocial,
    sort: f.sort || "new",
    max_rows: 40,
  });
...
/** The dropdown contents, built from what is open right now -- so the bar
 *  never offers a filter that can only return nothing. */
export async function publicPartyFacets(): Promise<Record<string, string[]>> {
  const { data, error } = await getSupabase().rpc("public_party_facets");
  if (error) return { genre: [], rating: [], decade: [], language: [] };
  return (data as any) ?? {};
}

export interface PrivateParty {
  join_code: string; title: string | null; content_id: string;
  host_user_id: string; host_name: string | null; host_handle: string | null;
  started_at: string; ended_at: string | null;
  requires_approval: boolean; is_host: boolean; joined_count: number;
}

/** Private rooms the caller has an actual relationship with: they hosted it, or
 *  they were let into it. A private room is in no directory and no search, so
 *  this is the only surface that can show one at all. */
export async function myPrivateParties(): Promise<PrivateParty[]> {
  const { data, error } = await getSupabase().rpc("my_private_parties");
  if (error) { console.warn("[party] private list", error); return []; }
  return (data ?? []) as PrivateParty[];
}

export interface ComingUp {
  host_user_id: string; handle: string | null; name: string;
  weekday: number | null; hour: number | null; tz: string | null;
  next_at: string | null; reminded: boolean; live: boolean;
}

/** Derived from each followed host's one slot, projected to the next
 *  occurrence. There is no schedule table and there should not be one: nobody
 *  has promised to be anywhere. */
export async function comingUp(): Promise<ComingUp[]> {
  const { data, error } = await getSupabase().rpc("coming_up");
  if (error) { console.warn("[party] coming up", error); return []; }
  return (data ?? []) as ComingUp[];
}

/** Remind me is a reminder about a HABIT, not a scheduled event. It fires when
 *  the host actually starts something and stays silent when they do not --
 *  "okra usually goes live about now" is a notification about an absence, and
 *  the first two of those teach someone to mute the channel. */
export async function setReminder(hostId: string, on: boolean): Promise<boolean> {
  const sb = getSupabase();
  const { data: u } = await sb.auth.getUser();
  if (!u.user) return false;
  const { error } = on
    ? await sb.from("party_reminders").upsert(
        { user_id: u.user.id, host_id: hostId }, { onConflict: "user_id,host_id" })
    : await sb.from("party_reminders").delete()
        .eq("user_id", u.user.id).eq("host_id", hostId);
  if (error) { console.warn("[party] reminder", error); return false; }
  return true;
}

export interface BlockedPerson {
  user_id: string; handle: string | null; name: string;
  reason: "fit" | "conduct"; party_title: string | null; created_at: string;
  note: string | null; appeal: string | null; appeal_at: string | null;
}

/** The host's own blocks. Nobody else can read this list, including the people
 *  on it: knowing exactly who has blocked you is an invitation to work around
...
export async function myBlocks(): Promise<BlockedPerson[]> {
  ...
  if (error) { console.warn("[party] blocks", error); return []; }
  return (data ?? []) as BlockedPerson[];
}

/** Answering closes the request either way. Keeping someone blocked does not
 *  notify them -- deliberately no feedback that would reward trying again. */
export async function answerAppeal(userId: string, lift: boolean): Promise<boolean> {
  const { data, error } = await getSupabase().rpc("answer_appeal", { subject: userId, lift });
  if (error) { console.warn("[party] appeal", error); return false; }
  if (lift) { try { socket?.send(JSON.stringify({ type: "unban", userId })); } catch {} }
  return (data as any)?.ok !== false;
}

/** A private annotation on a person, shown to exactly one reader: its author.
 *  Passing an empty string clears it. */
export async function setPartyNote(userId: string, note: string): Promise<boolean> {
  const { data, error } = await getSupabase().rpc("set_party_note", { subject: userId, body: note });
  if (error) { console.warn("[party] note", error); return false; }
  return (data as any)?.ok !== false;
}

/** One short message from a blocked person to the host who blocked them. The
 *  answer is the same whether it was sent, already sent, or the block never
 *  existed -- a distinguishable refusal is a probe. */
export async function sendBlockAppeal(hostId: string, message: string): Promise<boolean> {
  const { data, error } = await getSupabase().rpc("send_block_appeal", { host: hostId, body: message });
  if (error) { console.warn("[party] send appeal", error); return false; }
  return (data as any)?.ok !== false;
}
